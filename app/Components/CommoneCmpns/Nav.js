...
const Nav = () => {
    ...
    return (
        <div id='nav-main-con' className='flex items-center justify-between px-[7vmin] py-[3vmin] bg-black'>
            <svg id='logo' width="72" height="30" viewBox="0 0 72 30" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <div id='nav-con' className={`transition-all nav-item flex ${open === 0 ? "right-[-100%]" : "right-[0%]"} ${shut === 0 ? "right-[0%]" : "right-[-100%]"} items-center justify-center gap-[3.7vmin]`}>
                {["services", "works", "insight", "about us", "contuct",].map((e, inx) => {
                    return <h3 id='contunct' className={` item cursor-pointer font-light capitalize text-[2.4vmin] ${inx === 4 && 'ml-[25vmin]'}`}>{e}</h3>
                })}
            </div>
            <div id='menu-btn' onClick={openMenu} className="hidden absolute right-[6%] top-[12%] close-btn text-[7.6vmin] cursor-pointer">
                <TfiAlignRight />
            </div>
            <div onClick={shut} className={`${open === 1 ? "block" : "hidden"} z-[1000] bg-red-600 rounded-full p-[2vmin] absolute right-[6%] top-[12%]close-btn text-[4vmin] cursor-pointer`}>
                <TfiClose />
            </div>
        </div>
    )
